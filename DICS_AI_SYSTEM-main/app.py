...
def assign_task(response_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('incident_response_detail', response_id=response_id))
    
    return render_template('pages/assign_task.html', response=response)
...
def allocate_resource(response_id):
    ...
    return render_template('pages/allocate_resource.html', response=response)
...
def create_situation_report(response_id):
    ...
    if request.method == 'POST':
        ...
        # Update incident response summary
    ...
    return render_template('pages/create_situation_report.html', response=response)
...
@app.route('/incident-response/<int:response_id>/close', methods=['POST'])
def close_incident_response(response_id):
    if not is_incident_commander():
        return jsonify({'error': 'Unauthorized'}), 403
    ...
    flash('Incident response closed successfully', 'success')
    return redirect(url_for('incident_commander_dashboard'))
